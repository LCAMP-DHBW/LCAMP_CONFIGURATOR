...
async function sendActionPartList() {
    const msg = {
        sender: document.getElementById('sender').value,
        command: "Action",
        content: "PartList",
...
async function sendSystemConfiguration() {
    const checkedValues = Array.from(
        document.querySelectorAll('input[name="optionType"]:checked')
    ).map(el => el.value);

    const systemData = {
        Controller: document.getElementById('sys_controler').value,
        Battery: document.getElementById('sys_akku').value,
        Bottom: document.getElementById("sys_botton").value,
        Central: document.getElementById('sys_center').value,
...
async function sendColorSchemeConfiguration() {
    const colorShemeData = {
        Base: document.getElementById('baseColor').value,
        Main: document.getElementById('mainColor').value,
        Detail: document.getElementById("detailColor").value,
    ...
    const msg = {
        ...
        content: document.getElementById('content').value,
        type: "setColorScheme",
        colorSchemeConfiguration: colorShemeData
    };
    await sendToServer(msg);
...
async function View3DFile() {
    const canvas = document.getElementById('colorCanvas');
    const previewImg = document.getElementById("colorPreviewImage");
    console.log("colorPreviewImage:", previewImg);
    //    await new Promise(resolve => setTimeout(resolve, 100));
    const timestamp = new Date().getTime();
    previewImg.src = `mobilerobot.png?_=${timestamp}`; // Datei im gleichen Ordner wie HTML
    previewImg.alt = "Mobiler Roboter Vorschau";
